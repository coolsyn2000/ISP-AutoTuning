...
def demo_test_raw():
    cfg = Config('./fast_OpenISP/configs/test.yaml')
    ...
    output_path = op.join(OUTPUT_DIR, './fast_OpenISP/test.png')
    output = cv2.cvtColor(data['output'], cv2.COLOR_RGB2BGR)
    cv2.imwrite(output_path, output)


def demo_nikon_d3x():
    cfg = Config('./fast_OpenISP/configs/nikon_d3x.yaml')
    pipeline = Pipeline(cfg)

    pgm_path = './fast_OpenISP/raw/color_checker.pgm'
